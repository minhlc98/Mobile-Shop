...
public class Activity_Authentication extends AppCompatActivity {
    ...
    public void Create_OrderID(final String TenKH, final String DienThoai, final String DiaChi, final String Email) {
        ...
        dialog.show();
        RetrofitClient.getInstace().getService().createOrderID(TenKH,DienThoai,DiaChi,Email).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    String id_donhang=response.body().string();
                    if (id_donhang!="fail") Create_Order_Detail(id_donhang,dialog);
                    else{
                        dialog.cancel();
    ...
    public void Create_Order_Detail(final String id_donhang, final Dialog dialog){
        RetrofitClient.getInstace().getService().createOrderDetail(createJSON(id_donhang)).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    dialog.cancel();
    ...
    public void event(){
        ...
        btnXacThuc.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction()==MotionEvent.ACTION_DOWN) btnXacThuc.setBackgroundResource(R.color.green);
                if (event.getAction()==MotionEvent.ACTION_UP) btnXacThuc.setBackgroundResource(R.color.blue);
                return false;
            }
        });
        btnGuiLai.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction()==MotionEvent.ACTION_DOWN) btnGuiLai.setBackgroundResource(R.color.green);
                if (event.getAction()==MotionEvent.ACTION_UP) btnGuiLai.setBackgroundResource(R.color.blue);
                return false;
            }
        });
    }
